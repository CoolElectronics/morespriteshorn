    ...
    if button == 1 then
        if not app.toolMenuX then
            ...
            for i, room in ipairs(project.rooms) do
                if mx >= room.x and mx <= room.x + room.w * 8 and my >= room.y and
            ...
            elseif app.tool == "select" then
                ...
                if not project.selection then
                    ...
                    project.selectionMoveX, project.selectionMoveY = mx -
                                                                         project.selection
                                                                             .x,
                                                                     my -
                                                                         project.selection
                                                                             .y
                    project.selectionStartX, project.selectionStartY =
                        project.selection.x, project.selection.y
                end
            end
        end
        ...
        if love.keyboard.isDown("lalt") and app.room then
            app.roomResizeSideX = sign(
                                      mx - activeRoom().x - activeRoom().w * 8 /
                                          2)
            app.roomResizeSideY = sign(
                                      my - activeRoom().y - activeRoom().h * 8 /
                                          2)
            return
        end

        if app.tool == "brush" then app.brushing = true end
    ...
    if app.tool == "rectangle" or app.tool == "select" then
        ...
        elseif app.tool == "select" then
            ...
            if project.selection and project.selectionMoveX then
                if project.selection.x == project.selectionStartX and
                    project.selection.y == project.selectionStartY then
                    placeSelection()
                end
            end
        end
...
function love.wheelmoved(x, y)
    ...
    if app.anyWindowHovered then if ui:wheelmoved(x, y) then return end end
    ...
    if y ~= 0 then
        ...
        app.camScale = app.camScaleSetting > 0 and (app.camScaleSetting + 1) or
                           2 ^ app.camScaleSetting
